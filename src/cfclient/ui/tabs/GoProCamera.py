...
from cfclient.ui.tabs import constants
...
class GoPro:
    ...
    def getMediaFusion(self):
        ...
        try:
            ...
            return ["http://" + self.ip_addr + "/videos/DCIM/" + folder_1 + "/" + file_1,
            ...
            if folder == "" and file == "":
                if self.getMediaInfo("file").endswith("MP4"):
                    return json.loads(self._request(
                        "gp/gpMediaMetadata?p=" + self.getMediaInfo("folder") + "/" + self.getMediaInfo(
                            "file") + "&t=videoinfo"))
        else:
            data = ""
            if folder == "" and file == "":
    ...
    def parse_value(self, param, value):
        ...
        if param == "rem_space":
            if value == 0:
                return "No SD"
            ammnt = 1000
            if self.whichCam() == constants.Camera.Interface.GPControl and self.infoCamera(
    ...
    def overview(self):
        if self.whichCam() == constants.Camera.Interface.GPControl:
            ...
            print("space left in sd card: " + "" + self.parse_value("rem_space",
                                                                    self.getStatus(constants.Status.Status,
                                                                                   constants.Status.STATUS.RemainingSpace)))
            ...
            print("Is Recording: " + "" + self.parse_value("recording",
                                                           self.getStatus(constants.Status.Status,
                                                                          constants.Status.STATUS.IsRecording)))
